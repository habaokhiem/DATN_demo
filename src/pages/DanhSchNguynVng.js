import "./DanhSchNguynVng.css";
const DanhSchNguynVng = ({ goToDanhSachNguyenVongDeXuat }) => {
  return (
    <div className="danhsachnguyenvong">
      <div className="group-parent10">
        <div className="frame-parent735">
          <div className="vuesaxlineararrow-right-wrapper22">
            <img
              className="vuesaxlineararrow-right-icon30"
              alt=""
              src="./vuesaxlineararrowright61.svg"
            />
          </div>
          <div className="frame-parent736">
            <div className="frame-parent737">
              <div className="vuesaxlinearhome-parent9">
                <img
                  className="vuesaxlinearhome-icon11"
                  alt=""
                  src="./vuesaxlinearhome21.svg"
                />
                <div className="text143">Trang chủ</div>
              </div>
              <div className="vuesaxlinearhome-parent9">
                <img
                  className="vuesaxlinearhome-icon11"
                  alt=""
                  src="./vuesaxlinearreceiptedit1.svg"
                />
                <div className="text143">Khảo sát</div>
              </div>
              <div className="iconoutlinedapplicationbulb-parent9">
                <img
                  className="iconoutlinedapplicationbulb11"
                  alt=""
                  src="./iconoutlinedapplicationbulb.svg"
                />
                <div className="text143">Gợi ý nguyện vọng</div>
              </div>
              <div className="vuesaxlinearhome-parent9">
                <img
                  className="vuesaxlinearhome-icon11"
                  alt=""
                  src="./vuesaxlinearinfocircle11.svg"
                />
                <div className="text143">Thông tin tuyển sinh</div>
              </div>
              <div className="vuesaxlinearhome-parent9">
                <img
                  className="vuesaxlinearhome-icon11"
                  alt=""
                  src="./vuesaxlinearlogout2.svg"
                />
                <div className="text143">Sign Out</div>
              </div>
            </div>
            <div className="female15-parent19">
              <img
                className="female15-icon21"
                alt=""
                src="./female151@2x.png"
              />
              <div className="text148">User1</div>
            </div>
          </div>
        </div>
        <div className="notifications19">
          <div className="title19">
            <div className="text148">Thông báo</div>
          </div>
          <div className="frame-parent738">
            <div className="tt-c-wrapper17">
              <div className="tt-c19">Tất cả</div>
            </div>
            <div className="cha-c-wrapper17">
              <div className="tt-c19">Chưa đọc</div>
            </div>
          </div>
          <div className="frame-parent739">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">59 phút trước</div>
              </div>
            </div>
            <div className="frame-child496" />
          </div>
          <div className="frame-parent740">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">1 Giờ trước</div>
              </div>
            </div>
            <div className="frame-child498" />
          </div>
          <div className="frame-parent741">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">Hôm nay, 11:59 AM</div>
              </div>
            </div>
            <div className="frame-child496" />
          </div>
          <div className="frame-parent742">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">Hôm nay, 11:59 AM</div>
              </div>
            </div>
            <div className="frame-child498" />
          </div>
          <div className="frame-parent743">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">Hôm nay, 11:59 AM</div>
              </div>
            </div>
            <div className="frame-child496" />
          </div>
          <div className="frame-parent744">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">Hôm nay, 11:59 AM</div>
              </div>
            </div>
            <div className="frame-child498" />
          </div>
          <div className="frame-parent745">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">Hôm nay, 11:59 AM</div>
              </div>
            </div>
            <div className="frame-child498" />
          </div>
          <div className="frame-parent746">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">Hôm nay, 11:59 AM</div>
              </div>
            </div>
            <div className="frame-child498" />
          </div>
          <div className="frame-parent747">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">Hôm nay, 11:59 AM</div>
              </div>
            </div>
            <div className="frame-child496" />
          </div>
          <div className="frame-parent748">
            <div className="rectangle-parent281">
              <img
                className="frame-child495"
                alt=""
                src="./rectangle-484@2x.png"
              />
              <div className="byewind-parent186">
                <div className="byewind188">Người dùng mới đăng ký</div>
                <div className="emailgmailcom19">Hôm nay, 11:59 AM</div>
              </div>
            </div>
            <div className="frame-child496" />
          </div>
          <div className="button19">
            <img
              className="dotsthreeoutlinevertical-s-icon19"
              alt=""
              src="./dotsthreeoutlineverticals.svg"
            />
          </div>
        </div>
        <div className="frame-wrapper165">
          <div className="frame-wrapper166">
            <div className="frame-parent749">
              <div className="gi-nguyn-vng-wrapper4">
                <div className="gi-nguyn6">Gợi ý nguyện vọng</div>
              </div>
              <div className="iconoutlinedapplicationbell-wrapper22">
                <img
                  className="vuesaxlinearhome-icon11"
                  alt=""
                  src="./iconoutlinedapplicationbell.svg"
                />
              </div>
            </div>
          </div>
        </div>
        <div className="frame-parent750">
          <div className="frame-parent751">
            <div className="frame-parent752">
              <div className="frame-parent753">
                <div
                  style={styles.button}
                  onClick={() => {
                    goToDanhSachNguyenVongDeXuat();
                  }}
                  className="danh-sch-nguyn-vng-xut-wrapper2"
                >
                  <div className="danh-sch-nguyn10">
                    Danh sách nguyện vọng đề xuất
                  </div>
                </div>
                <div
                  style={styles.button}
                  className="danh-sch-nguyn-vng-hin-ti-wrapper2"
                >
                  <div className="gi-nguyn6">
                    Danh sách nguyện vọng hiện tại
                  </div>
                </div>
              </div>
              <i className="y-l-phc6">
                *Đây là phục vụ cho việc tham khảo cho học sinh.
              </i>
            </div>
            <div className="frame-parent754">
              <div className="i-hc-fpt-c-s-h-ni-container">
                <div className="i-hc-bch-container2">
                  <ul className="i-hc-bch15">Đại học Bách khoa Hà Nội</ul>
                </div>
                <div className="frame-parent755">
                  <div className="m-ngnh-parent56">
                    <div className="m-ngnh58">Mã ngành</div>
                    <div className="tn-ngnh61">Tên ngành</div>
                    <div className="khi-thi59">Khối thi</div>
                  </div>
                  <div className="frame-parent756">
                    <div className="frame-parent749">
                      <div className="nth0414">NTH04</div>
                      <div className="ngn-ng-anh38">Ngôn ngữ Anh</div>
                      <div className="d0130">D01</div>
                    </div>
                    <div className="frame-parent749">
                      <div className="nth0414">NTH05</div>
                      <div className="ngn-ng-anh38">Ngôn ngữ Pháp</div>
                      <div className="d0130">D01, D03</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="i-hc-fpt-c-s-h-ni-container">
                <div className="i-hc-bch-container2">
                  <ul className="i-hc-bch15">Đại học Kinh tế Quốc dân</ul>
                </div>
                <div className="frame-parent755">
                  <div className="m-ngnh-parent56">
                    <div className="m-ngnh58">Mã ngành</div>
                    <div className="tn-ngnh61">Tên ngành</div>
                    <div className="khi-thi59">Khối thi</div>
                  </div>
                  <div className="frame-parent756">
                    <div className="frame-parent749">
                      <div className="nth0414">NTH01-02</div>
                      <div className="ngn-ng-anh38">Kinh tế quốc tế</div>
                      <div className="d0130">{`A00, A01, D01, D03, D05, D06, D07 `}</div>
                    </div>
                    <div className="frame-parent749">
                      <div className="nth0414">NTH01-02</div>
                      <div className="ngn-ng-anh38">Kinh tế</div>
                      <div className="d0130">
                        A00, A01, D01, D03, D05, D06, D07
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="i-hc-fpt-c-s-h-ni-container">
                <div className="i-hc-bch-container2">
                  <ul className="i-hc-bch15">Đại học FPT (Cơ sở Hà Nội)</ul>
                </div>
                <div className="frame-parent755">
                  <div className="m-ngnh-parent56">
                    <div className="m-ngnh58">Mã ngành</div>
                    <div className="tn-ngnh61">Tên ngành</div>
                    <div className="khi-thi59">Khối thi</div>
                  </div>
                  <div className="frame-wrapper167">
                    <div className="nth01-01-parent14">
                      <div className="nth0414">NTH01-01</div>
                      <div className="ngn-ng-anh38">Luật</div>
                      <div className="d0130">D01</div>
                    </div>
                  </div>
                  <div className="frame-wrapper167">
                    <div className="nth01-01-parent14">
                      <div className="nth0414">NTH01-01</div>
                      <div className="ngn-ng-anh38">Luật</div>
                      <div className="d0130">D01</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="i-hc-fpt-c-s-h-ni-container">
                <div className="i-hc-bch-container2">
                  <ul className="i-hc-bch15">
                    Đại học Ngoại thương (Cơ sở Hà Nội)
                  </ul>
                </div>
                <div className="frame-parent755">
                  <div className="m-ngnh-parent56">
                    <div className="m-ngnh58">Mã ngành</div>
                    <div className="tn-ngnh61">Tên ngành</div>
                    <div className="khi-thi59">Khối thi</div>
                  </div>
                  <div className="frame-wrapper167">
                    <div className="nth01-01-parent14">
                      <div className="nth0414">NTH02</div>
                      <div className="ngn-ng-anh38">Quản trị khách sạn</div>
                      <div className="d0130">
                        A00, A01, D01, D03, D05, D06, D07
                      </div>
                    </div>
                  </div>
                  <div className="frame-wrapper167">
                    <div className="nth01-01-parent14">
                      <div className="nth0414">NTH02</div>
                      <div className="ngn-ng-anh38">Quản trị khách sạn</div>
                      <div className="d0130">
                        A00, A01, D01, D03, D05, D06, D07
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="i-hc-fpt-c-s-h-ni-container">
                <div className="i-hc-bch-container2">
                  <ul className="i-hc-bch15">Đại học Thương mại</ul>
                </div>
                <div className="frame-parent755">
                  <div className="m-ngnh-parent56">
                    <div className="m-ngnh58">Mã ngành</div>
                    <div className="tn-ngnh61">Tên ngành</div>
                    <div className="khi-thi59">Khối thi</div>
                  </div>
                  <div className="frame-wrapper167">
                    <div className="nth01-01-parent14">
                      <div className="nth0414">NTH02</div>
                      <div className="ngn-ng-anh38">Quản trị khách sạn</div>
                      <div className="d0130">
                        A00, A01, D01, D03, D05, D06, D07
                      </div>
                    </div>
                  </div>
                  <div className="frame-wrapper167">
                    <div className="nth01-01-parent14">
                      <div className="nth0414">NTH02</div>
                      <div className="ngn-ng-anh38">Quản trị khách sạn</div>
                      <div className="d0130">
                        A00, A01, D01, D03, D05, D06, D07
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="frame-parent762">
            <div style={styles.button} className="thay-i-thuc-tnh-wrapper4">
              <div className="gi-nguyn6">Thay đổi thuộc tính</div>
            </div>
            <div style={styles.button} className="xem-kt-qu-xut-wrapper2">
              <div className="gi-nguyn6">Xem kết quả đề xuất</div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

const styles = {
  button: {
    cursor: "pointer",
    userSelect: "none",
  },
};

export default DanhSchNguynVng;
